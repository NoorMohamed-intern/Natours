...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 3) if everything is ok, send responce to client
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(new AppError('Please login to get access to all Tours', 401));
  }

  //2 Verification token

  const decoded = await promisify(jwt.verify)(token, process.env.JWT_SECRET);
  // console.log(`brr😃: ${decoded.id}`);

  //3 Check if user still token`
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  //2) Check if token is expired and there is user, set the new password
